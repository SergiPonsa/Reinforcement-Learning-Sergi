...
class Actor(nn.Module): #Neural net to choose the action taking into acount the state
    ...
    def __init__(self, state_size, action_size, seed, fc1_units=259 , fc2_units = 259): #original only 1 hidden layer 256 neurons
    ...
    def forward(self, state):
        ...
        state[2] = state[2]/8 # to normalize to 1
...
class Critic(nn.Module): # Evaluate the selected action for that state
    ...
    def __init__(self, state_size, action_size, seed, fcs1_units=259, fc2_units=259, fc3_units=259):
        """Initialize parameters and build model.
        Params
        ======
            state_size (int): Dimension of each state
    ...
    def forward(self, state, action):
        ...
        xs = F.relu(self.fcs1(state)) # Allow mainly positive values for the states, but some negative too
        x = torch.cat((xs, action), dim=1)  # Put together the action input with the output of the previous layer
        x = F.relu(self.fc2(x)) # Allow mainly positive values for the states, but some negative too
        x = F.relu(self.fc3(x)) # Allow mainly positive values for the states, but some negative too
        return self.fc4(x)
